...
from typing import Callable, List, Optional, Text, Dict, Any, Type
...
async def test_train_model_checkpointing(
    ...
    with default_model_storage.read_from(default_diet_resource) as model_dir:
        ...
        there must be at least the following:
            - metadata.json
            - checkpoint
            - component_1_CountVectorsFeaturizerGraphComponent (as per the pipeline above)
            - component_2_DIETClassifierGraphComponent files (more than 1 file)
        """
...
async def test_adjusting_layers_incremental_training(
    ...
    """Tests adjusting sparse layers of `DIETClassifierGraphComponent` to increased sparse
